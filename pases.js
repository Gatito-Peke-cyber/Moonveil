/* =========================================================
   Moonveil Portal — pases.js v5.0
   · Estilo gamer pixel art (mismo que tienda)
   · Color índigo/violeta característico de pases
   · Firebase sync en tiempo real (onSnapshot)
   · freeRewards[] y paidRewards[] vacíos — rellenar tú mismo
   ========================================================= */
...
async function doFirebaseSync() {
  ...
  try {
    ...
    await updateDoc(doc(db, 'users', currentUID), {
      ...
      pass_mail_claimed: JSON.parse(localStorage.getItem(PASS_MAIL_LS) || '[]'),
...
async function loadFromFirebase(uid) {
  try {
    ...
    applyRemotePassData(snap.data());
  } catch (e) { console.warn('[Pases] load:', e); }
}

function applyRemotePassData(data) {
  let changed = false;
  if (data.pass_states) {
...
/* ══════════════════════════════════════════════════════════
   ...
   ⚠️ DEJA ESTOS ARRAYS VACÍOS — tú los rellenarás
══════════════════════════════════════════════════════════ */
function makeDefaultLevels() {
  const levels = [];

  const freeRewards = [
    // Pon aquí tus recompensas gratis, una por nivel (índice 0 = nivel 1)
    // Ejemplo: { type:'keys', emoji:'⭐', name:'Llave Superestrella', amount:1, keyId:'key_common' }
  ];

  const paidRewards = [
    // Pon aquí tus recompensas de pago, una por nivel (índice 0 = nivel 1)
...
const PASSES = [
  ...
  {
    id:'pass_s10', name:'Pesadilla del Nether', season:'Temporada X',          emoji:'🔥', description:'El Nether se despierta.',        startDate:'2026-10-01', endDate:'2026-10-31', music:'music/8.mp3', bg:'img-pass/banhall.jpg',    shopItemId:'s10',
    levels: (() => {
      const ls = makeDefaultLevels();
  ...
  {
    id:'pass_s12', name:'Estrella de Ender', season:'Temporada XII',           emoji:'⭐', description:'La Estrella de Ender brilla.',    startDate:'2026-12-01', endDate:'2026-12-31', music:'music/8.mp3', bg:'img-pass/chrismine.jpg',  shopItemId:'s12',
    levels: (() => {
      const ls = makeDefaultLevels();
...
function getPassState(passId) {
  try {
    ...
    return raw ? { ...PASS_STATE_DEFAULTS, ...JSON.parse(raw) } : { ...PASS_STATE_DEFAULTS };
  } catch { return { ...PASS_STATE_DEFAULTS }; }
}
...
/* ══════════════════════════════════════════════════════════
   SISTEMA DE BOOSTS
══════════════════════════════════════════════════════════ */
function getActiveBoosts() { return lsGet(PASS_BOOSTS_LS, []).filter(b => b.expiry > now()); }
...
function processReward(passId, passName, reward, mult = 1) {
  ...
  switch (r.type) {
    case 'xp':      addXP(passId, amt); addToInventory(passId, passName, 'xp', '⭐', 'XP Bonus', amt); break;
    case 'emeralds':addToInventory(passId, passName, 'emeralds', '💎', 'Gemas', amt); break;
    case 'keys': {
    ...
    case 'tickets': awardTicket(r.subtype || 'classic', amt); addToInventory(passId, passName, `ticket_${r.subtype||'classic'}`, '🎫', r.name || 'Ticket', amt); break;
    case 'copper':  addToInventory(passId, passName, 'copper', '🪙', 'Monedas', amt); break;
    case 'boost':   activateBoost(passId, r.boostHours || 1, r.boostMultiplier || 1.5, r.name); addToInventory(passId, passName, `boost_${now()}`, '⚡', r.name || 'Boost XP', 1); break;
    case 'random': {
      const opts = [
    ...
    default: addToInventory(passId, passName, `item_${now()}`, r.emoji || '📦', r.name || 'Ítem', amt);
  }
}
...
function checkMissionReset(passId, m) {
  ...
  const n = now();
  if (!mst.resetAt) {
    const ns = { progress:m.preCompleted?m.target:0, claimed:false, resetAt:getNextReset(m.reset) };
    saveMissionState(passId, m.id, ns); return;
  }
  if (n > mst.resetAt) {
    const ns = { progress:m.preCompleted?m.target:0, claimed:false, resetAt:getNextReset(m.reset) };
    saveMissionState(passId, m.id, ns);
  }
}
...
function isMissionUnlocked(passId, m) {
  ...
  const st = getPassState(passId);
  return TIER_ORDER.indexOf(st.tier) >= TIER_ORDER.indexOf(m.exclusive);
}

function isTierOwned(st, tierId) { return TIER_ORDER.indexOf(tierId) <= TIER_ORDER.indexOf(st.tier || 'stone'); }
...
function renderPassHeader(passId) {
  ...
  tickH(); _headerCDIv = setInterval(tickH, 1000);
}
...
function renderTrack(passId) {
  ...
  const paidColor = paidColors[st.tier] || paidColors.stone;
  ...
  if (selBar) {
    selBar.innerHTML = TIERS.map(t => {
      ...
      const isAct = st.tier === t.id;
      return `<button class="tier-pill ${t.id} ${isAct?'active':''} ${!owned?'locked':''}" onclick="filterTierView('${passId}','${t.id}')">
  ...
  if (claimAllWrap) {
    if (pendingTrack > 0) {
      claimAllWrap.innerHTML = `<button class="claim-all-btn" onclick="claimAllLevelRewards('${passId}')">✨ RECLAMAR TODO <span class="claim-all-badge">${pendingTrack}</span></button>`;
      claimAllWrap.style.display = '';
    } else { claimAllWrap.innerHTML = ''; claimAllWrap.style.display = 'none'; }
  }
  ...
  for (let i = 0; i < MAX_LEVELS; i++) {
    ...
    const freeR  = lvlData.free, paidR = lvlData.paid;
    ...
    freeRow += `<div class="track-node-cell ${isMile?'milestone-cell':''} ${isCur?'current-cell':''}">
      <div class="${fnc}" onclick="openLevelModal(${i},'free')">
    ...
    paidRow += `<div class="track-node-cell ${isMile?'milestone-cell':''} ${isCur?'current-cell':''}">
      <div class="${pnc}" onclick="openLevelModal(${i},'paid')">
  ...
  setTimeout(() => {
    ...
    ['trackHScroll','trackFScroll','trackPScroll'].forEach(id => { const el=document.getElementById(id); if(el) el.scrollLeft=idx*88; });
  }, 150);
}
...
function renderMaxLevelBanner(passId) {
  ...
  const ovArr = getOverflowRewards(st.tier);
...
function filterTierView(passId, tierId) {
  const st = getPassState(passId);
  if (!isTierOwned(st, tierId)) { toast(`🔒 Necesitas el Pase ${TIERS.find(t=>t.id===tierId)?.name||tierId}`); return; }
  ...
  if (bodyEl) bodyEl.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:10px">
      <div style="padding:18px;background:rgba(156,163,175,0.06);border:2px solid ${clFree?'rgba(52,211,153,0.4)':'rgba(156,163,175,0.25)'};box-shadow:3px 3px 0 #000;text-align:center;position:relative">
        ...
        ${canClaimFree?`<button style="margin-top:10px;height:34px;padding:0 14px;font-family:var(--font-pixel);font-size:0.28rem;background:linear-gradient(135deg,#6b7280,#9ca3af);color:#fff;border:2px solid var(--tier-stone);box-shadow:2px 2px 0 #000;cursor:pointer" onclick="claimLevelReward(${levelIndex},'free')">⬜ RECLAMAR</button>`:''}
      ...
      <div style="padding:18px;background:${hasPaid?'rgba(251,191,36,0.06)':'rgba(0,0,0,0.15)'};border:2px solid ${clPaid?'rgba(52,211,153,0.4)':'rgba(251,191,36,0.3)'};box-shadow:3px 3px 0 #000;text-align:center;position:relative${!hasPaid?';filter:grayscale(0.7);opacity:0.6':''}">
        ...
        ${canClaimPaid?`<button style="margin-top:10px;height:34px;padding:0 14px;font-family:var(--font-pixel);font-size:0.28rem;background:linear-gradient(135deg,#f59e0b,#fbbf24);color:#000;border:2px solid var(--tier-gold);box-shadow:2px 2px 0 #000;cursor:pointer" onclick="claimLevelReward(${levelIndex},'paid')">✨ RECLAMAR</button>`:''}
...
function claimAllLevelRewards(passId) {
  ...
  for (let i = 0; i < MAX_LEVELS; i++) {
    ...
    if (!freshSt.claimedLevels.includes(`free_${i}`) && lvlData.free) {
      ...
      const s2 = getPassState(passId); s2.claimedLevels=[...(s2.claimedLevels||[]),`free_${i}`]; savePassState(passId, s2); total++;
    }
    if (hasPaid) {
      ...
      if (!s3.claimedLevels.includes(`paid_${i}`) && lvlData.paid) {
        ...
        const s4 = getPassState(passId); s4.claimedLevels=[...(s4.claimedLevels||[]),`paid_${i}`]; savePassState(passId, s4); total++;
      }
    }
  }
  toast(total > 0 ? `✨ ¡${total} recompensas reclamadas!` : '📭 No hay recompensas pendientes');
  renderPassHeader(passId); renderTrack(passId); renderInventory(passId); updateHUD(passId);
}
...
function renderMissions(passId) {
  ...
  if (status === 'upcoming') {
    ...
    renderBan(); _missBanIv = setInterval(renderBan, 1000);
    container.innerHTML = ''; return;
  }
  ...
  if (claimAllMWrap) {
    if (claimable.length > 0) {
      claimAllMWrap.innerHTML = `<button class="claim-all-btn" onclick="claimAllMissions('${passId}')">📋 RECLAMAR TODO <span class="claim-all-badge">${claimable.length}</span></button>`;
      claimAllMWrap.style.display = '';
    } else { claimAllMWrap.innerHTML = ''; claimAllMWrap.style.display = 'none'; }
  }
  ...
  const timeLeft = ts => {
    ...
    if(d>=1) return `${d}d ${h}h`; if(h>=1) return `${h}h ${m}m`; return `${m}m`;
  };
  ...
  for (const [cat, {icon, missions}] of Object.entries(groups)) {
    ...
    missions.forEach(m => {
      ...
      const cdLabel = mst.resetAt && !claimed && m.reset !== 'unique' && now() < mst.resetAt ? `<span class="mc-cooldown">↻ en ${timeLeft(mst.resetAt)}</span>` : '';
      ...
      html += `<div class="mission-card ${claimed?'completed':''} ${!unlocked?'locked':''} ${m.exclusive?'exclusive':''} ${m.isGame?'game-mission':''}">
        ...
        <div class="mc-reward-col">
          ...
          ${claimed?'<span class="mc-claimed">✓ HECHO</span>':canClaim?`<button class="mc-claim-btn" onclick="claimMission('${m.id}')">✨ RECLAMAR</button>`:`<button class="mc-claim-btn" disabled>${pct}%</button>`}
        </div>
      </div>`;
    });
    html += `</div></div>`;
...
function claimMission(mId) {
  ...
  const st = getPassState(activePassId);
  if (!st.stats) st.stats = {};
  if (m.reset === '7d') { st.stats.weeklyMissionsDone = (st.stats.weeklyMissionsDone||0)+1; savePassState(activePassId, st); }
...
function claimAllMissions(passId) {
  ...
  MISSIONS_TEMPLATE.forEach(m => {
    ...
    const st = getPassState(passId);
    if (!st.stats) st.stats = {};
    if (m.reset === '7d') { st.stats.weeklyMissionsDone = (st.stats.weeklyMissionsDone||0)+1; savePassState(passId, st); }
    trackMissionProgress(passId, 'weekly_missions', 1);
    total++;
  });
  toast(total > 0 ? `✅ ¡${total} misiones reclamadas!` : '📭 No hay misiones por reclamar');
  renderMissions(passId); renderPassHeader(passId); renderInventory(passId); updateHUD(passId);
}
...
function renderGlobalInventory() {
  ...
  const byPass = {};
  entries.forEach(([, item]) => { const pn=item.passName||'Global'; if(!byPass[pn])byPass[pn]=[]; byPass[pn].push(item); });
...
function renderMejoras(passId) {
  ...
  container.innerHTML = TIER_UPGRADES.map(u => {
    ...
    const isStone = u.id === 'stone', isIron = u.id === 'iron';
    ...
    if (isStone) {
      ...
      footerHtml = `<div class="mejora-price-wrap"><span class="mejora-price-label">Precio</span><span class="mejora-price">⟡${u.price}</span></div><button class="mejora-buy-btn" onclick="redirectToShop('${passId}')">🛒 IR A LA TIENDA</button>`;
    } else if (!hasReq) {
      ...
      footerHtml = `<div class="mejora-price-wrap"><span class="mejora-price-label">Precio</span><span class="mejora-price">⟡${u.price}</span></div><button class="mejora-buy-btn" onclick="buyTierUpgrade('${passId}','${u.id}')">⬆️ ACTIVAR TIER</button>`;
    }
...
function buyTierUpgrade(passId, tierId) {
  ...
  if (u.requiresPrev && !isTierOwned(st, u.requiresPrev)) {
    toast(`⚠️ Necesitas el ${TIER_UPGRADES.find(t=>t.id===u.requiresPrev)?.name||u.requiresPrev} primero`); return;
  }
  if (TIER_ORDER.indexOf(tierId) > TIER_ORDER.indexOf(st.tier)) {
    st.tier = tierId; savePassState(passId, st);
    toast(`✨ ¡${u.name} activado!`);
    renderMejoras(passId); renderTrack(passId); renderPassHeader(passId); updateHUD(passId); renderMissions(passId);
  }
}
...
window.activatePassTier = function(passId, tierId) {
  ...
  if (TIER_ORDER.indexOf(tierId) > TIER_ORDER.indexOf(st.tier||'stone')) {
    ...
    if (activePassId === passId) { renderMejoras(passId); renderTrack(passId); renderPassHeader(passId); updateHUD(passId); renderMissions(passId); }
  }
};
    ...
    return `<div class="pmail-item ${isClaimed?'claimed':'unread'}">
      ...
      <div class="pmail-action">
        ${isClaimed
          ...
          :`<button class="btn-pixel btn-pass btn-sm" onclick="claimPassMail('${item.id}')">🎁 RECLAMAR</button>`}
      </div>
    </div>`;
  }).join('');
}
...
function claimPassMail(mailId) {
  ...
  const pass = PASSES.find(p => p.id === activePassId);
  claimed.push(mailId); lsSet(PASS_MAIL_LS, claimed);
  item.rewards.forEach(r => {
    switch (r.type) {
      case 'xp':      addXP(activePassId, r.amount||0); addToInventory(activePassId, pass.name, 'xp_mail', '⭐', 'XP Bonus', r.amount||0); break;
      case 'keys':    awardChestKey(r.keyId, r.amount||1); addToInventory(activePassId, pass.name, `key_${r.keyId}`, r.emoji||'🔑', r.name||'Llave', r.amount||1); toast(`✨ ${r.emoji} ${r.name} ×${r.amount}`); break;
      case 'boost':   activateBoost(activePassId, r.boostHours||1, r.boostMultiplier||1.5, r.name); break;
      case 'emeralds':addToInventory(activePassId, pass.name, 'emeralds', '💎', 'Gemas', r.amount||0); toast(`💎 +${r.amount} Gemas`); break;
      case 'copper':  addToInventory(activePassId, pass.name, 'copper', '🪙', 'Monedas', r.amount||0); toast(`🪙 +${r.amount} Monedas`); break;
    }
  });
  toast(`📬 ¡${item.title}!`);
  scheduleSync();
  renderPassMailbox(); renderPassHeader(activePassId); renderInventory(activePassId); updateHUD(activePassId);
}
...
function updateHUD(passId) {
  ...
  if (boostWrap) {
    ...
    } else boostWrap.classList.add('hidden');
  }
}
...
function renderPassSelector() {
  ...
  track.innerHTML = PASSES.map((p, idx) => {
    const sMs = new Date(p.startDate+'T00:00:00').getTime(), eMs = new Date(p.endDate+'T23:59:59').getTime();
    ...
    return `<button class="pass-card-btn" data-pass-id="${p.id}" onclick="setActivePass('${p.id}')" style="animation-delay:${idx*0.04}s">
...
(function particles() {
  ...
  const init = () => {
    ...
    parts = Array.from({length:80}, () => ({ x:Math.random()*w, y:Math.random()*h, r:(.3+Math.random()*1.2)*dpi, s:.12+Math.random()*.4, a:.03+Math.random()*.12, hue:220+Math.random()*60 }));
  };
  const tick = () => {
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🏆 Moonveil Pases v5.0 — Pixel Gamer Edition');
