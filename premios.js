/**
 * premios.js — Sistema Gacha Moonveil v3.3
 * ════════════════════════════════════════
 * ✅ Fix misiones: reclamar una no reclama todas
 * ✅ Fix misiones: sincronización entre dispositivos sin dar tickets doble
 * ✅ Fix eco onSnapshot: _locallyCompletedMissions evita el rebote
 * ✅ Tickets, inventario, pity sincronizados entre dispositivos
 */
...
/*
 * Set de IDs de misiones completadas en ESTE dispositivo durante esta sesión.
 * Evita que el eco del onSnapshot vuelva a procesar estas misiones
 * como "remotas nuevas" y las marque/dé tickets extra.
 */
const _locallyCompletedMissions = new Set();
...
async function doSync(){
  ...
  try{
    ...
    await updateDoc(doc(db,'users',currentUID),{
      ...
      gacha_stats:       stats,
      gacha_tickets:     tix,
      gacha_total_spins: spins,
      gacha_pity:        pityData,
      gacha_missions:    missionsState,
      gacha_sync_ts:     _syncTimestamp,
      updatedAt:         serverTimestamp(),
    });
    setTimeout(()=>{ _syncInFlight=false; }, 2500);
  }catch(e){
...
/* ══════════════════════════════════════
   APPLY REMOTE DATA
   Aplica cambios del OTRO dispositivo.
   Misiones: marca como completadas SIN dar tickets.
══════════════════════════════════════ */
function applyRemoteData(data){
  ...
  if(data.gacha_pity){
    ROULETTES.forEach(r=>{
      const re=data.gacha_pity[`pity_epic_${r.id}`]??0;
      const rl=data.gacha_pity[`pity_legend_${r.id}`]??0;
      if(re>getPityEpic(r.id)){ localStorage.setItem(LS.pityEpic(r.id),   String(re)); changed=true; }
      if(rl>getPityLeg(r.id)) { localStorage.setItem(LS.pityLegend(r.id), String(rl)); changed=true; }
    });
  }

  // ── Misiones: marcar como completadas SIN dar tickets ──
  // Solo procesa misiones que NO reclamamos nosotros en esta sesión.
  if(data.gacha_missions){
    ...
    Object.keys(data.gacha_missions).forEach(missionId=>{
      // Si la reclamamos nosotros, ignorar (es nuestro eco)
      if(_locallyCompletedMissions.has(missionId)) return;

      const remote=data.gacha_missions[missionId];
      const localM=local[missionId];

      // Solo aplicar si el remoto la tiene completada y el local no
      if(remote?.completedAt && !localM?.completedAt){
        local[missionId]=remote;
        // También registrar en el set local para no re-procesar
        _locallyCompletedMissions.add(missionId);
        missionChanged=true;
        console.log(`[Gacha] Misión ${missionId} recibida de otro dispositivo (sin tickets)`);
      }
    });

    if(missionChanged){
      // Guardar en LS directamente, sin scheduleSync para no crear bucle
      try{ localStorage.setItem(LS.missions, JSON.stringify(local)); }catch{}
      changed=true;
    }
  }
...
function startRealtimeListener(uid){
  ...
  _gachaUnsub=onSnapshot(
    ...
    (snap)=>{
      ...
      if(_syncInFlight){ console.log('[Gacha] Snapshot ignorado — sync en curso'); return; }
      const snapTs=snap.data()?.gacha_sync_ts||0;
      if(snapTs===_syncTimestamp && _syncTimestamp>0){ console.log('[Gacha] Eco propio — ignorado'); return; }
      applyRemoteData(snap.data());
    },
    (err)=>{ console.warn('[Gacha] onSnapshot error:',err); }
  );

  console.log('[Gacha] ✅ Listener activo');
}
...
async function loadFromFirebase(uid){
  try{
    ...
    if(d.gacha_pity){
      ROULETTES.forEach(r=>{
        const re=d.gacha_pity[`pity_epic_${r.id}`]??0;
        const rl=d.gacha_pity[`pity_legend_${r.id}`]??0;
        if(re>getPityEpic(r.id)) localStorage.setItem(LS.pityEpic(r.id),  String(re));
        if(rl>getPityLeg(r.id))  localStorage.setItem(LS.pityLegend(r.id), String(rl));
      });
    }

    // Misiones: cargar desde Firebase, fusionar con local SIN dar tickets
    if(d.gacha_missions){
      const local=lsGet(LS.missions,{});
      Object.keys(d.gacha_missions).forEach(mid=>{
        ...
        if(remote?.completedAt && !local[mid]?.completedAt){
          local[mid]=remote;
          // Marcar como ya procesada para el onSnapshot posterior
          _locallyCompletedMissions.add(mid);
        }
      });
...
/* ══════════════════════════════════════
   GIROS TOTALES
══════════════════════════════════════ */
function getTotalSpins(id){ return Math.max(0,parseInt(localStorage.getItem(LS.totalSpins(id))||'0',10)); }
function incrementTotalSpins(id,count=1){ localStorage.setItem(LS.totalSpins(id),String(getTotalSpins(id)+count)); }
...
function showInventory(){
  ...
  if(!items.length){
    openModal(`<h2>INVENTARIO — ${wname}</h2><p style="font-family:var(--font-vt);font-size:1.15rem;color:var(--muted);text-align:center;padding:30px">Aún no has ganado nada aquí.<br>¡Gira la ruleta!</p><div style="text-align:center;margin-top:14px"><button class="btn-pixel btn-purple" onclick="document.getElementById('modal').setAttribute('aria-hidden','true')">CERRAR</button></div>`);
    return;
  }
  const order={legend:0,epic:1,rare:2,uncommon:3,common:4};
  items.sort((a,b)=>(order[a.rarity]??5)-(order[b.rarity]??5));
  const cards=items.map(i=>`<div class="prize-card ${i.rarity}" style="padding:14px;text-align:center"><div class="prize-img">${i.img}</div><div class="prize-name">${i.label}</div><div class="prize-rarity" style="color:${rarityColor(i.rarity)}">${i.rarity.toUpperCase()}</div><div style="font-family:var(--font-pixel);font-size:0.55rem;color:#c4b5fd;margin-top:8px">x${i.count}</div></div>`).join('');
  openModal(`<h2>INVENTARIO — ${wname}</h2><p style="font-family:var(--font-vt);font-size:1.1rem;color:var(--muted);text-align:center;margin-bottom:14px">${items.length} tipos · ${items.reduce((s,i)=>s+i.count,0)} ítems totales</p><div class="prize-grid-modal">${cards}</div><div style="display:flex;gap:10px;justify-content:center;margin-top:16px;flex-wrap:wrap"><button class="btn-pixel btn-ghost" id="btnClearInv">🗑️ LIMPIAR INVENTARIO</button><button class="btn-pixel btn-purple" onclick="document.getElementById('modal').setAttribute('aria-hidden','true')">CERRAR</button></div>`);
  setTimeout(()=>{ $('#btnClearInv')?.addEventListener('click',()=>{ closeModal(); clearWheelInventory(wid); }); },50);
}

/* ══════════════════════════════════════
   ÚLTIMOS PREMIOS
══════════════════════════════════════ */
function saveLastPrizes(rewards){ lsSet(LS.lastPrizes, rewards.map(r=>({img:r.img,label:r.label,rarity:r.rarity,desc:r.desc}))); }
function renderLastPrizesPanel(rewards){
  const c=$('#lastPrize'); if(!c) return;
  if(!rewards||!rewards.length){ c.innerHTML=`<div class="no-prize"><div class="no-prize-ico">?</div><p>AÚN NO HAS GIRADO</p></div>`; return; }
  if(rewards.length===1){
    const r=rewards[0];
    c.innerHTML=`<div class="prize-card ${r.rarity}"><div class="prize-img">${r.img}</div><div class="prize-name">${r.label}</div><div class="prize-rarity" style="color:${rarityColor(r.rarity)}">${r.rarity.toUpperCase()}</div></div>`;
  } else {
    const cards=rewards.map(r=>`<div class="last-prize-mini ${r.rarity}" title="${r.label}"><div class="lpm-img">${r.img}</div><div class="lpm-name">${r.label}</div><div class="lpm-rar" style="color:${rarityColor(r.rarity)}">${r.rarity.slice(0,3).toUpperCase()}</div></div>`).join('');
...
function pickWithPity(wheelId){
  ...
  const epics=avail.filter(r=>r.rarity==='epic'), legends=avail.filter(r=>r.rarity==='legend');
  ...
  if(pl>=45&&legends.length){ const extra=(pl-44)*0.06; if(Math.random()<extra){ const r=legends[Math.floor(Math.random()*legends.length)]; setPityEpic(wheelId,pe+1); setPityLeg(wheelId,0); return r; } }
  ...
  for(const r of avail){
    rng-=r.weight;
    if(rng<=0){ if(r.rarity==='legend'){setPityLeg(wheelId,0);setPityEpic(wheelId,pe+1);}else if(r.rarity==='epic'){setPityEpic(wheelId,0);setPityLeg(wheelId,pl+1);}else{setPityEpic(wheelId,pe+1);setPityLeg(wheelId,pl+1);} return r; }
  }
...
function renderWheel(wheelId){
  ...
  bgC.setAttribute('cx',cx); bgC.setAttribute('cy',cy); bgC.setAttribute('r',r); bgC.setAttribute('fill','#0d0b15'); svg.appendChild(bgC);
  wheel.rewards.forEach((reward,i)=>{
    ...
    path.setAttribute('fill',colorMap[reward.rarity]||'rgba(255,255,255,0.04)'); path.setAttribute('stroke','rgba(139,92,246,0.2)'); path.setAttribute('stroke-width','1.5'); svg.appendChild(path);
    const line=document.createElementNS('http://www.w3.org/2000/svg','line');
    line.setAttribute('x1',cx); line.setAttribute('y1',cy); line.setAttribute('x2',x1); line.setAttribute('y2',y1);
    line.setAttribute('stroke','rgba(139,92,246,0.3)'); line.setAttribute('stroke-width','2'); svg.appendChild(line);
    const mid=(sa+ea)/2, tx=cx+(r*0.68)*Math.cos(mid), ty=cy+(r*0.68)*Math.sin(mid);
    const emoji=document.createElementNS('http://www.w3.org/2000/svg','text');
    emoji.setAttribute('x',tx); emoji.setAttribute('y',ty); emoji.setAttribute('text-anchor','middle'); emoji.setAttribute('dominant-baseline','middle'); emoji.setAttribute('font-size','18'); emoji.textContent=reward.img||'?'; svg.appendChild(emoji);
  });
  const edge=document.createElementNS('http://www.w3.org/2000/svg','circle');
  edge.setAttribute('cx',cx); edge.setAttribute('cy',cy); edge.setAttribute('r',r-2); edge.setAttribute('stroke','rgba(139,92,246,0.6)'); edge.setAttribute('stroke-width','4'); edge.setAttribute('fill','none'); svg.appendChild(edge);
...
function checkWheelLock(wheel){
  ...
  if(!isWheelActive(wheel)){ lockEl.hidden=false; const ts=getTimeUntilStart(wheel),te=getTimeUntilEnd(wheel); if(ts&&ts>0){txt.textContent='PRÓXIMAMENTE';rangeEl.textContent=`Inicia en ${formatMs(ts)}`;}else{txt.textContent='FINALIZADA';rangeEl.textContent=wheel.end?new Date(wheel.end+'T23:59:59').toLocaleDateString('es-ES'):'—';} } else { lockEl.hidden=true; }
}
...
async function spinWheel(times=1){
  ...
  beep(880,0.05,0.06);
  clearTimeout(syncTimeout); doSync();
}
...
function showPrizeModal(reward){
  openModal(`<div style="text-align:center;padding:20px"><div style="font-size:80px;margin-bottom:14px">${reward.img}</div><h2 style="color:${rarityColor(reward.rarity)};text-shadow:2px 2px 0 #000">${reward.label}</h2><p style="font-family:var(--font-vt);font-size:1.2rem;color:var(--muted);margin:10px 0">${reward.desc}</p><span style="display:inline-block;font-family:var(--font-pixel);font-size:0.34rem;padding:5px 14px;border:2px solid ${rarityColor(reward.rarity)};color:${rarityColor(reward.rarity)};background:rgba(0,0,0,0.4)">${reward.rarity.toUpperCase()}</span><div style="margin-top:24px"><button class="btn-pixel btn-purple" onclick="document.getElementById('modal').setAttribute('aria-hidden','true')">¡GENIAL!</button></div></div>`);
}
function showMultiPrizeModal(rewards){
  const cards=rewards.map(r=>`<div class="prize-card-sm ${r.rarity}"><div class="pcm-img">${r.img}</div><div class="pcm-name">${r.label}</div><div class="pcm-rarity" style="color:${rarityColor(r.rarity)}">${r.rarity.toUpperCase()}</div></div>`).join('');
  openModal(`<h2>TIRADA x${rewards.length}</h2><div class="prize-grid-modal">${cards}</div><div style="text-align:center"><button class="btn-pixel btn-purple large" onclick="document.getElementById('modal').setAttribute('aria-hidden','true')">CONTINUAR</button></div>`);
}

function showPreview(){
  const wheel=ROULETTES.find(w=>w.id===currentWheelId); if(!wheel)return;
  const cards=wheel.rewards.map(r=>{ const s=getStock(r.id),stk=s!==null?`Stock: ${s}`:'Ilimitado'; return`<div class="prize-card-sm ${r.rarity}"><div class="pcm-img">${r.img}</div><div class="pcm-name">${r.label}</div><div class="pcm-rarity" style="color:${rarityColor(r.rarity)}">${r.rarity.toUpperCase()}</div><div style="font-family:var(--font-pixel);font-size:0.22rem;color:var(--muted);margin-top:4px">${stk}</div><div style="font-family:var(--font-vt);font-size:0.9rem;color:var(--dim)">${r.desc}</div></div>`; }).join('');
  openModal(`<h2>PREMIOS — ${wheel.title.toUpperCase()}</h2><div class="prize-grid-modal">${cards}</div><div style="text-align:center"><button class="btn-pixel btn-purple" onclick="document.getElementById('modal').setAttribute('aria-hidden','true')">CERRAR</button></div>`);
}

function showRules(){
  openModal(`<h2>REGLAS DEL SISTEMA GACHA</h2><div class="rules-body"><h3>TICKETS</h3><ul><li>Cada ruleta tiene sus propios tickets independientes</li><li>Se compran en la <a href="tienda.html" style="color:var(--a)">Tienda</a> y llegan automáticamente</li><li>También se ganan completando misiones</li></ul><h3>PITY / GARANTÍA</h3><ul><li><strong>Épico: garantizado cada 20 tiros</strong></li><li><strong>Legendario: puede salir antes de los 60 tiros</strong> (probabilidad creciente desde tiro 45)</li><li><strong>Hard pity legendario: tiro 60</strong> — siempre sale</li></ul><h3>MISIONES</h3><ul><li>Diarias: se resetean a las 00:00 hora local</li><li>Semanales: se resetean cada lunes a las 00:00</li><li>Si reclamas una misión en un dispositivo, el otro la marca como completada automáticamente</li><li>Los tickets solo se dan una vez, en el dispositivo donde se reclamó</li></ul><h3>INVENTARIO</h3><ul><li>Cada ruleta tiene su propio inventario</li><li>Puedes limpiar el inventario de cada ruleta por separado</li></ul></div><div style="text-align:center;margin-top:18px"><button class="btn-pixel btn-purple large" onclick="document.getElementById('modal').setAttribute('aria-hidden','true')">ENTENDIDO</button></div>`);
}
...
function renderWheelTabs(){
  ...
  ROULETTES.forEach(wheel=>{
    ...
    tab.innerHTML=`<span class="tab-ico">${wheel.icon}</span><span class="tab-name">${wheel.title.toUpperCase()}</span><span class="tab-desc">${wheel.desc}</span>${timerHTML}<span class="tab-status ${active?'active':'locked'}"></span>`;
...
function renderTicketsDisplay(){
  ...
  ROULETTES.forEach(wheel=>{ const badge=document.createElement('div'); badge.className='ticket-badge'; badge.innerHTML=`<div class="tb-ico">${wheel.icon}</div><div class="tb-info"><div class="tb-name">${wheel.title.toUpperCase()}</div><div class="tb-count">${getTickets(wheel.id)}</div></div>`; box.appendChild(badge); });
  renderHUDTickets();
}
function renderHUDTickets(){
  const box=$('#hudTickets'); if(!box)return; box.innerHTML='';
  ROULETTES.forEach(wheel=>{ const slot=document.createElement('div'); slot.className='hud-ticket-slot'; slot.title=`Tickets ${wheel.title}`; slot.innerHTML=`<span class="t-ico">${wheel.icon}</span><span>${getTickets(wheel.id)}</span>`; box.appendChild(slot); });
}

/* ══════════════════════════════════════
   MISIONES
   REGLAS:
   - completeMission() = dar tickets + guardar en LS + registrar en _locallyCompletedMissions + sync a FB
   - applyRemoteData() = solo marcar como completada en LS, SIN tickets
   - _locallyCompletedMissions filtra el eco del onSnapshot
══════════════════════════════════════ */
...
function renderMissions(){
  ...
  if(wheel&&!isWheelActive(wheel)&&getTimeUntilStart(wheel)&&getTimeUntilStart(wheel)>0){
    box.innerHTML=`<div class="missions-locked-msg"><div class="mlm-ico">🔒</div><div class="mlm-txt">MISIONES BLOQUEADAS</div><div class="mlm-sub">La ruleta aún no ha iniciado. Las misiones estarán disponibles cuando abra.</div></div>`;
  ...
  let header='';
  if(hasDailies){ const sec=Math.floor(msUntilMidnight()/1000); header+=`<div class="mission-reset-row daily-reset"><span class="mcd-label">DIARIAS RESET:</span><span class="mcd-val" id="mcd-daily">${formatTime(sec)}</span></div>`; }
  if(hasWeeklies){ const sec=Math.floor(msUntilMonday()/1000); header+=`<div class="mission-reset-row weekly-reset"><span class="mcd-label">SEMANALES RESET:</span><span class="mcd-val" id="mcd-weekly">${formatTime(sec)}</span></div>`; }
  ...
  missions.forEach(m=>{
    ...
    // El listener SÍ pasa el objeto de misión completo para dar los tickets correctos
...
/**
 * Reclamar misión — flujo correcto:
 * 1. Verificar que no esté ya completada (guarda contra doble click)
 * 2. Registrar en _locallyCompletedMissions ANTES de hacer nada más
 *    → así cuando llegue el eco del onSnapshot, lo ignorará
 * 3. Guardar en localStorage
 * 4. Dar tickets (solo aquí, nunca en applyRemoteData)
 * 5. Sync inmediato a Firebase
 * 6. Re-renderizar
 */
function completeMission(id, freq, count){
  if(isMissionDone(id,freq)){ toast('Misión ya completada','✓'); return; }

  // Paso 2: registrar ANTES para proteger el eco
  _locallyCompletedMissions.add(id);

  // Paso 3: guardar en localStorage directamente (sin scheduleSync para no interferir)
  const state=getMissionsState();
  state[id]={ completedAt:Date.now(), resetDate:freq==='daily'?todayStr():weekStr(), freq };
  try{ localStorage.setItem(LS.missions, JSON.stringify(state)); }catch{}

  // Paso 4: dar tickets
  addTickets(currentWheelId, count);

  // Paso 5: sync inmediato
  clearTimeout(syncTimeout);
  doSync();
...
function updateBoostUI(){
  ...
  if(boostState.activeUntil>now){ const diff=Math.floor((boostState.activeUntil-now)/1000); btn.disabled=true;btn.textContent='ACTIVO x2';timer.textContent=formatTime(diff);status.textContent='ACTIVO x2';TICKET_MULT=2; }
  else if(boostState.cooldownUntil>now){ const diff=Math.floor((boostState.cooldownUntil-now)/1000); btn.disabled=true;btn.textContent='COOLDOWN';timer.textContent=formatTime(diff);status.textContent='COOLDOWN';TICKET_MULT=1; }
  else{ btn.disabled=false;btn.textContent='ACTIVAR x2';timer.textContent='';status.textContent='DISPONIBLE';TICKET_MULT=1; }
}
...
══════════════════════════════════════ */
function boot(){
  console.log('🎡 Moonveil Gacha v3.3 — Mission Fix');
  ...
  onAuthChange(async user=>{
    if(!user){ if(_gachaUnsub){_gachaUnsub();_gachaUnsub=null;} return; }
    currentUID=user.uid;
  ...
  window.addEventListener('beforeunload',()=>{ if(_gachaUnsub){_gachaUnsub();_gachaUnsub=null;} });
}
