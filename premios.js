 ...
 * ════════════════════════════════════════
 * ✅ TICKETS: se sincronizan en tiempo real (bajan al girar en otro dispositivo)
 * ✅ MISIONES: reclamar en un dispositivo bloquea en todos los demás
 * ✅ PITY: sincronizado exacto entre dispositivos
 * ✅ INVENTARIO: fusión acumulativa cross-device
 * ✅ onSnapshot activo — igual que contactos y perfil
 */
...
/* ══════════════════════════════════════
   ...
   ══════════════════════════════════════
   FIXES APLICADOS:
   1. Tickets: se usa el valor EXACTO de Firebase, no Math.max
      (al girar los tickets BAJAN, Math.max los mantendría altos)
   2. Pity: igual que tickets, valor exacto de Firebase
   3. Misiones: se guardan en Firebase; al recibirlas via snapshot
      se marcan como hechas localmente => no se pueden reclamar dos veces
══════════════════════════════════════ */
let currentUID = null, syncTimeout = null;
let _gachaUnsub = null;
let _isSyncing  = false;   // true mientras este dispositivo está subiendo datos
...
async function doSync(){
  ...
  try{
    ...
    await updateDoc(doc(db,'users',currentUID), {
      ...
      gacha_tickets:   tix,
      gacha_pity:      pityData,
      gacha_missions:  missions,
      updatedAt:       serverTimestamp(),
    });
  } catch(e){
    console.warn('[Gacha] sync error:', e);
  } finally {
    setTimeout(() => { _isSyncing = false; }, 2000);
  }
}

/* ── Aplicar datos remotos (otro dispositivo) ── */
function applyRemoteData(data){
  let changed = false;

  /* TICKETS: valor exacto de Firebase
     Si el otro dispositivo giró, los tickets BAJARON en Firebase.
     Hay que reflejar eso aunque localmente tengamos más. */
  if(data.gacha_tickets){
    ROULETTES.forEach(r => {
      const remote = data.gacha_tickets[r.id] ?? 0;
      const local  = getTickets(r.id);
      if(remote !== local){
        localStorage.setItem(LS.tickets(r.id), String(Math.max(0, remote)));
        changed = true;
      }
    });
  }

  /* INVENTARIO: fusionar (tomar el mayor count por ítem)
     Los ítems solo se acumulan, nunca bajan */
  ...
  /* MISIONES: si el remoto las tiene completadas, marcarlas localmente
     Así reclamar en la laptop bloquea en el móvil */
  if(data.gacha_missions){
    const local  = lsGet(LS.missions, {});
    const merged = { ...local };
    let missionChanged = false;
    Object.keys(data.gacha_missions).forEach(id => {
      const rm = data.gacha_missions[id];
      const lm = local[id];
      if(rm && rm.completedAt){
        if(!lm || !lm.completedAt || rm.completedAt > (lm.completedAt||0)){
          merged[id]     = rm;
          missionChanged = true;
        }
      }
...
/* ── Carga inicial (Firebase = fuente de verdad al abrir) ── */
async function loadFromFirebase(uid){
  try{
    ...
    if(d.gacha_tickets){
      ROULETTES.forEach(r => {
        const remote = d.gacha_tickets[r.id] ?? 0;
        localStorage.setItem(LS.tickets(r.id), String(Math.max(0, remote)));
      });
    }
    ...
    if(d.gacha_pity){
      ROULETTES.forEach(r => {
        localStorage.setItem(LS.pityEpic(r.id),   String(d.gacha_pity[`pity_epic_${r.id}`]  ?? 0));
        localStorage.setItem(LS.pityLegend(r.id),  String(d.gacha_pity[`pity_legend_${r.id}`]?? 0));
      });
    }
    ...
    if(d.gacha_missions){
      const local  = lsGet(LS.missions, {});
      const merged = { ...local };
      Object.keys(d.gacha_missions).forEach(id => {
        const rm = d.gacha_missions[id];
        const lm = local[id];
        if(rm && rm.completedAt){
          if(!lm || !lm.completedAt || rm.completedAt > (lm.completedAt||0)){
            merged[id] = rm;
  ...
  } catch(e){ console.warn('[Gacha] loadFromFirebase:', e); }
}
...
const ROULETTES = [
  ...
  { id:'dark_moon', title:'Luna Oscura', icon:'🌑', desc:'Temporada de Luna Oscura. Premios de las sombras.', bg:null, music:null, start:`${YR}-03-01`, end:`${YR}-04-30`,
    rewards:[
  ...
  { id:'spring', title:'Primavera Mágica', icon:'🌸', desc:'Flores encantadas y premios de temporada.', bg:null, music:null, start:`${YR}-05-01`, end:`${YR}-06-30`,
    rewards:[
  ...
  { id:'storm', title:'Tormenta Eléctrica', icon:'⚡', desc:'El poder del rayo en tus manos.', bg:null, music:null, start:`${YR}-01-01`, end:`${YR}-03-31`,
    rewards:[
  ...
  { id:'cyber', title:'Cyber Gacha', icon:'🤖', desc:'Premios del mundo digital. Abre en julio.', bg:null, music:null, start:`${YR}-07-01`, end:`${YR}-08-31`,
    rewards:[
  ...
  { id:'abyss', title:'Abismo Eterno', icon:'🕳️', desc:'Lo más oscuro del portal.', bg:null, music:null, start:`${YR}-09-01`, end:`${YR}-12-31`,
    rewards:[
...
function showInventory(){
  ...
  if(!items.length){
    openModal(`<h2>INVENTARIO — ${wname}</h2>
  ...
  setTimeout(()=>{
    $('#btnClearInv')?.addEventListener('click',()=>{ closeModal(); clearWheelInventory(wid); });
  }, 50);
}
...
function renderLastPrizesPanel(rewards){
  ...
  if(rewards.length===1){
    const r = rewards[0];
    c.innerHTML = `<div class="prize-card ${r.rarity}">
...
async function spinWheel(times=1){
  ...
  // Descontar tickets INMEDIATAMENTE (antes de la animación)
  // Esto asegura que Firebase recibe los tickets ya descontados
...
function showMultiPrizeModal(rewards){
  ...
  openModal(`<h2>TIRADA x${rewards.length}</h2>
    <div class="prize-grid-modal">${cards}</div>
...
function showPreview(){
  ...
  openModal(`<h2>PREMIOS — ${wheel.title.toUpperCase()}</h2>
    <div class="prize-grid-modal">${cards}</div>
    <div style="text-align:center">
      <button class="btn-pixel btn-purple" onclick="document.getElementById('modal').setAttribute('aria-hidden','true')">CERRAR</button>
    </div>`);
...
function showRules(){
  openModal(`<h2>REGLAS DEL SISTEMA GACHA</h2>
    <div class="rules-body">
      <h3>TICKETS</h3>
      <ul>
        <li>Cada ruleta tiene sus propios tickets independientes</li>
        <li>Se compran en la <a href="tienda.html" style="color:var(--a)">Tienda</a></li>
      ...
      <ul>
        ...
        <li><strong>Legendario: soft pity desde tiro 45</strong></li>
        <li><strong>Hard pity legendario: tiro 60</strong></li>
      </ul>
      <h3>MISIONES</h3>
      <ul>
        <li>Diarias: se resetean a las 00:00 hora local</li>
        <li>Semanales: se resetean cada lunes</li>
        <li>Reclamar en un dispositivo bloquea la misión en todos los demás</li>
      </ul>
    </div>
...
function renderMissions(){
  ...
  if(wheel&&!isWheelActive(wheel)&&getTimeUntilStart(wheel)&&getTimeUntilStart(wheel)>0){
    box.innerHTML=`<div class="missions-locked-msg">
...
function completeMission(id, freq, count){
  ...
  // Sync a Firebase rápido (500ms) para que el otro dispositivo lo vea pronto
  scheduleSync(500);
}
...
/* ══ BOOT ══ */
function boot(){
  console.log('🎡 Moonveil Gacha v3.2 — Cross-Device Sync');
  ...
  onAuthChange(async user=>{
    ...
    // 1. Carga inicial — Firebase es la fuente de verdad
    ...
    // 2. Listener en tiempo real — detecta cambios del otro dispositivo
