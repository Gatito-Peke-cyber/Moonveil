/* ================================================================
   Moonveil Portal — Contactos v4.1
   UPDATED: pass tier badges (iron/gold/emerald/diamond) junto al nombre
   · El nombre cambia de color según el tier activo del pase
   · Una medalla aparece al lado del nombre
   · Se actualiza en tiempo real desde Firestore
   · Desaparece automáticamente cuando expiresAt < now()
   ================================================================ */
...
const CLOUDINARY_PRESET = 'moonveil_uploads';
...
const PASS_TIER_META = {
  iron:    { emoji: '⬛', label: 'HIERRO',    cssClass: 'tier-iron'    },
  gold:    { emoji: '🟨', label: 'ORO',       cssClass: 'tier-gold'    },
  emerald: { emoji: '🟩', label: 'ESMERALDA', cssClass: 'tier-emerald' },
  diamond: { emoji: '🔷', label: 'DIAMANTE',  cssClass: 'tier-diamond' },
};

/**
 * Extrae el tier activo de los datos de un usuario de Firestore.
 * Devuelve null si no tiene pase pagado activo o si ya expiró.
 * @param {object} userData — datos del documento Firestore del usuario
 * @returns {{ tierId, passId, passName, expiresAt } | null}
 */
function getActiveTierFromData(userData) {
  if (!userData) return null;
  const pt = userData.active_pass_tier;
  if (!pt || !pt.tierId || pt.tierId === 'stone') return null;
  if (!PASS_TIER_META[pt.tierId]) return null; // tier desconocido
  const nowIso = new Date().toISOString();
  if (pt.expiresAt && pt.expiresAt < nowIso) return null; // ya expiró
  return pt;
}

/**
 * Genera el HTML de la medalla de pase para un tier dado.
 * @param {{ tierId, passName, expiresAt } | null} tierData
 * @returns {string} HTML string
 */
function buildTierBadgeHTML(tierData) {
  if (!tierData) return '';
  const meta = PASS_TIER_META[tierData.tierId];
  if (!meta) return '';
  const expiryStr = tierData.expiresAt
    ? `Expira: ${tierData.expiresAt.slice(0, 10)}`
    : '';
  const title = [
    `${meta.label}`,
    tierData.passName ? `Pase: ${tierData.passName}` : '',
    expiryStr,
  ].filter(Boolean).join(' · ');
  return `<span class="ci-pass-badge ${meta.cssClass}" title="${esc(title)}">${meta.emoji} ${meta.label}</span>`;
}
...
function initStars() {
  ...
  const init = () => {
    ...
    stars = Array.from({length:100}, () => ({x:Math.random()*W, y:Math.random()*H, r:Math.random()*1.4+0.4, o:Math.random()*.4+.06, s:Math.random()*.3+.06, ci:Math.floor(Math.random()*COLS.length)}));
  };
  const tick = () => {
...
/* ── Timer para recheck de tiers expirados (cada 60 seg) ── */
let _tierExpiryCheckIv = null;

/* ── Scroll thread ── */
const scrollThread = () => requestAnimationFrame(() => { const t = $('#thread'); if (t) t.scrollTop = t.scrollHeight; });

/* ── Click en imágenes del chat ── */
...
onAuthStateChanged(auth, async user => {
  ...
  /* Chequeo periódico de tiers expirados — cada 60 s */
  if (_tierExpiryCheckIv) clearInterval(_tierExpiryCheckIv);
  _tierExpiryCheckIv = setInterval(() => {
    let changed = false;
    friendList.forEach(f => {
      const prev    = f.passtierId;
      const nowTier = getActiveTierFromData({ active_pass_tier: f._rawPassTier || null });
      const newId   = nowTier ? nowTier.tierId : null;
      if (prev !== newId) {
        f.passtierId    = newId;
        f.passtierData  = nowTier;
        changed = true;
      }
    });
...
async function setOnline() {
  if (!CU) return;
  try { await updateDoc(doc(db, 'users', CU.uid), {presence:{state: getS().lastSeen ? 'online' : 'hidden', section:'contactos', lastSeen:new Date().toISOString()}}); } catch {}
}
...
/* ══════════════════════════════════════
   LOAD FRIENDS
   · Incluye listener de active_pass_tier
══════════════════════════════════════ */
async function loadFriends() {
  ...
  try {
    ...
    friendList.forEach(f => {
      ...
      /* ── Listener de presencia + pass tier (mismo snapshot) ── */
      presenceUnsubs.push(onSnapshot(doc(db, 'users', f.uid), snap => {
        ...
        /* ── Actualizar tier del pase ── */
        const tierData = getActiveTierFromData(d);
        friendList[idx]._rawPassTier = d.active_pass_tier || null;
        friendList[idx].passtierId   = tierData ? tierData.tierId : null;
        friendList[idx].passtierData = tierData;

        renderFriends();
        if (currentId === f.id) updateChatHd(friendList[idx]);
      }));

      /* ── Listener de mensajes no leídos ── */
      ...
      presenceUnsubs.push(onSnapshot(
        ...
        snap => {
          ...
          if (friendList[idx].unread === count) return;
...
/**
 * Construye el objeto amigo desde los datos de Firestore.
 * Incluye passtierId y passtierData para las medallas.
 */
function mkFriend(uid, d) {
  ...
  return {
    ...
    unread:   0,
    /* tier del pase */
    _rawPassTier: d.active_pass_tier || null,
    passtierId:   tierData ? tierData.tierId : null,
    passtierData: tierData,
  };
}
...
/**
 * Renderiza la lista de amigos incluyendo medallas de tier de pase.
 */
function renderFriends() {
  const list = $('#friendList'), div = $('#dividerFriends'), wrap = $('#friendsScrollWrap'); if (!list) return;
  const q = ($('#searchContacts')?.value || '').toLowerCase();
  const filtered = friendList.filter(c => !q || c.name.toLowerCase().includes(q)).sort((a, b) => {
...
/**
 * Genera el HTML de un item de amigo con medalla de pase si corresponde.
 */
function friendItem(c) {
  const active   = currentId === c.id;
  const tier     = c.passtierId;           // null si no tiene pase activo pagado
  const tierCss  = tier ? `tier-${tier}` : '';
  const nameCls  = tier ? `tier-${tier}` : '';
  const avCls    = tier ? `ci-av ${tierCss}` : 'ci-av';
  ...
  const tierBadge  = buildTierBadgeHTML(c.passtierData);
  const unread     = c.unread ? `<span class="ci-unread fr">${c.unread > 99 ? '99+' : c.unread}</span>` : '';

  return `<li class="contact-item friend-t ${tierCss} ${active ? 'active' : ''}" data-id="${esc(c.id)}">
    <div class="${avCls}">${avHTML(c.avatar, 44)}<div class="ci-online-dot ${dotCls}"></div></div>
    <div class="ci-meta">
      <div class="ci-name">
        <span class="ci-name-text ${nameCls}">${esc(c.name)}</span>
        ${tierBadge}
...
function selectContact(id) {
  ...
  if (grp)       { grp.unread = 0; renderGroups(); currentGroupId = grp.id; openGroupChat(grp); }
    ...
    frn.unread = 0;
    setDoc(doc(db, 'chats', chatId(CU.uid, frn.uid)), {[`unread_${CU.uid}`]: 0}, {merge:true}).catch(() => {});
    renderFriends();
    openFriendChat(frn);
  }
...
function botReply(bot, text) {
  ...
  if (bot.kind === 'options') {
    const opt = bot.brain.opts.find(o => o.l.toLowerCase() === t);
    return opt ? (typeof opt.r === 'string' ? opt.r : opt.r) : bot.brain.fallback;
  }
  if (bot.kind === 'keyword') {
    for (const k in bot.brain.kw) { if (t.includes(k.toLowerCase())) return bot.brain.kw[k]; }
...
function openFriendChat(friend) {
  ...
  getDocs(query(collection(db, 'chats', cid, 'messages'), where('senderId', '==', friend.uid), where('read', '==', false)))
    .then(snap => { snap.forEach(d => updateDoc(d.ref, {read:true}).catch(() => {})); })
    .catch(() => {});

  let first = true;
  msgUnsub = onSnapshot(
    query(collection(db, 'chats', cid, 'messages'), orderBy('timestamp', 'asc'), limit(120)),
    snap => {
      if (first) {
        ...
        snap.docChanges().forEach(ch => {
          if (ch.type === 'added') {
            ...
            if (m.senderId === CU.uid && localSentIds.has(m._localId || 'X')) {
              localSentIds.delete(m._localId);
              const p = $(`[data-local-id="${CSS.escape(m._localId)}"]`);
              if (p) p.remove();
            }
    ...
    err => {
      console.error('msg listener', err);
      th.innerHTML = `<div class="chat-empty"><div class="ce-desc" style="color:var(--red)">Error de conexión</div></div>`;
    }
  );
    ...
    msgUnsub = onSnapshot(buildQuery(ordered),
      snap => {
        if (first) {
          ...
          const docs = ordered ? snap.docs : [...snap.docs].sort((a, b) => {
...
function appendMsg(msg, contact, chatType) {
  ...
  if (msg.timestamp?.toDate) {
    try { ts = new Intl.DateTimeFormat('es-PE', {hour:'2-digit', minute:'2-digit'}).format(msg.timestamp.toDate()); } catch {}
  }
...
function clearImgPreview() {
  ...
  const b = $('#imgPreviewBar'); if (b) b.classList.add('hidden');
  const i = $('#imgInput'); if (i) i.value = '';
}
...
msgInput?.addEventListener('keydown', e => {
  if (e.key === 'Enter' && !e.shiftKey && getS().enterSend) {
    e.preventDefault();
...
async function sendMsg(text) {
  ...
  if (currentType === 'friend') {
    if (file) {
      ...
      } catch (e) { toast(`❌ Error: ${e.code || 'revisa configuración de Cloudinary'}`, 'error'); removeOptimistic(localId); localSentIds.delete(localId); }
    } else if (text) {
...
function removeOptimistic(localId) {
  const el = $(`[data-local-id="${CSS.escape(localId)}"]`); if (el) el.remove();
}
...
function pushPeer(c, content) {
  ...
  if (c.name === 'me') {
    ...
    node.innerHTML = `<div class="m-av">${avHTML(c.avatar || '👤', 28)}</div><div class="bubble"><div class="m-text">${fmtText(typeof content === 'string' ? content : '')}</div><div class="m-meta"><span>${timeNow()}</span><span class="m-ticks sent">✓✓</span></div></div>`;
    th.appendChild(node); scrollThread(); return;
  }
...
document.addEventListener('click', e => {
  const ep = $('#emojiPanel');
  if (ep && !ep.contains(e.target) && e.target.id !== 'btnEmoji') ep.classList.add('hidden');
});
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') ['#contactModal','#settingsModal','#statusCreatorModal','#groupModal','#groupSettingsModal'].forEach(closeModal);
});
...
function openFriendInfo(c) {
  ...
  const tierData  = c.passtierData;
  const tierMeta  = tierData ? PASS_TIER_META[tierData.tierId] : null;
  const tierHtml  = tierMeta
    ? `<div class="mig-row"><strong>Pase:</strong> <span class="ci-pass-badge ${tierMeta.cssClass}">${tierMeta.emoji} ${tierMeta.label}</span>${tierData.passName ? ` · ${esc(tierData.passName)}` : ''}</div>`
    : '';
  $('#modalTitle').textContent = c.name.toUpperCase();
  $('#modalBody').innerHTML = `<div class="modal-info-grid">
    <div class="mig-block"><div class="mig-title">PERFIL</div>
...
function openBotInfo(c) {
  $('#modalTitle').textContent = c.name.toUpperCase();
  $('#modalBody').innerHTML = `<div class="modal-info-grid">
    <div class="mig-block"><div class="mig-title">PERFIL</div><div class="mig-row"><strong>Alias:</strong> ${esc(c.alias)}</div></div>
    <div class="mig-block"><div class="mig-title">ESTADO</div><div class="mig-row">${c.online ? '🟢 EN LÍNEA' : '⚫ DESCONECTADO'}</div></div>
    <div class="mig-block full"><div class="mig-title">DESC.</div><div class="mig-row">${esc(c.desc)}</div></div>
  </div>`;
  openModal('#contactModal');
}
...
function openGroupSettings(group) {
  ...
  if (leaveBtn)    { leaveBtn.style.display = isCreator ? 'none' : ''; leaveBtn.textContent = '🚪 SALIR DEL GRUPO'; }
  if (dissolveBtn) dissolveBtn.classList.toggle('hidden', !isCreator);
...
async function kickMemberFromGroup(groupId, uid) {
  try {
    ...
    if (idx >= 0) {
      ...
      const cnt = $('#gsMemberCount'); if (cnt) cnt.textContent = `(${groupList[idx].members.length})`;
      const gm  = $('#gsGroupMeta');   if (gm)  gm.textContent  = `${groupList[idx].members.length} miembros · eres el creador`;
      renderGroups();
    }
...
function renderGsAddList(group) {
  ...
  list.innerHTML = notIn.map(f => `<div class="gs-add-row" data-uid="${esc(f.uid)}">
  ...
  try {
    ...
    if (gsNewIconFile) {
      ...
      try { iconUrl = await uploadImg(gsNewIconFile, 'group_icons'); }
        ...
        toast(`❌ Error: ${uploadErr.code || uploadErr.message}`, 'error');
...
function renderGroupEmojiGrid() {
  ...
  $$('.group-em', grid).forEach(b => b.addEventListener('click', () => {
    groupIconEmoji = b.dataset.ge; groupIconFile = null;
    const p = $('#groupIconPreview'); if (p) { p.innerHTML = ''; p.textContent = groupIconEmoji; }
    $$('.group-em', grid).forEach(x => x.classList.remove('active')); b.classList.add('active');
  }));
}
...
function renderGroupSelectedChips() {
  ...
  cont.innerHTML = members.map(uid => {
    const f = friendList.find(x => x.uid === uid);
    return `<div class="gsc">${esc(f ? f.name : '?')}<button data-uid="${esc(uid)}">✕</button></div>`;
  }).join('');
  $$('.gsc button', cont).forEach(b => b.addEventListener('click', () => {
    groupSelectedMembers.delete(b.dataset.uid);
    renderGroupMembersList(); renderGroupSelectedChips();
  }));
}
...
$('#groupImgInput')?.addEventListener('change', e => {
  ...
  groupIconFile = f;
  const r = new FileReader();
  r.onload = ev => { const p = $('#groupIconPreview'); if (p) p.innerHTML = `<img src="${esc(ev.target.result)}" alt="" />`; };
  r.readAsDataURL(f);
});
...
function loadFriendStatuses() {
  ...
  statusUnsubs.push(onSnapshot(doc(db, 'userStatuses', CU.uid), snap => {
    ...
    myStatuses = snap.exists() ? (snap.data().stories || []).filter(s => new Date(s.expiresAt) > nowDate) : [];
    renderStoriesBar();
  }));
  friendList.forEach(f => {
    statusUnsubs.push(onSnapshot(doc(db, 'userStatuses', f.uid), snap => {
      const nowDate = new Date();
      friendStatuses[f.uid] = snap.exists() ? (snap.data().stories || []).filter(s => new Date(s.expiresAt) > nowDate) : [];
      renderStoriesBar();
    }));
  });
}
...
function renderStoriesBar() {
  ...
  const bg = {};
  BOT_STORIES.forEach(st => { if (!bg[st.contact]) bg[st.contact] = []; bg[st.contact].push(st); });
...
$('#statusCreatorPublish')?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (tab === 'text') {
      ...
      toast('📤 Subiendo imagen...', 'info');
      let url;
      try { url = await uploadImg(scImg, 'statuses'); }
      catch (uploadErr) { toast(`❌ Error al subir: ${uploadErr.code || uploadErr.message || 'inténtalo de nuevo'}`, 'error'); reset(); return; }
      const cap = $('#statusImgCaption')?.value.trim() || '';
      await createStatus('image', url, cap, '');
      closeModal('#statusCreatorModal');
      toast('✅ Estado publicado (15 días)');
    }
  } catch (e) { toast(`❌ Error: ${e.code || e.message || 'inténtalo de nuevo'}`, 'error'); }
  reset();
});
async function createStatus(type, content, caption, bgColor) {
  const id  = `st_${Date.now()}`, nowDate = new Date(), exp = new Date(nowDate.getTime() + 15 * 24 * 3600000);
  await setDoc(doc(db, 'userStatuses', CU.uid), {stories: arrayUnion({id, type, content, caption, bgColor, createdAt:nowDate.toISOString(), expiresAt:exp.toISOString(), views:[]})}, {merge:true});
}
...
Object.entries(fields2).forEach(([id, key]) => {
...
$$('.wp-btn').forEach(btn => btn.addEventListener('click', () => {
  $$('.wp-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active');
  const s = getS(); s.wallpaper = btn.dataset.wp; saveS(s);
}));
...
$('#settCopyPlayerId')?.addEventListener('click', () => {
  ...
  if (pid && pid !== '—') navigator.clipboard?.writeText(pid).then(() => toast('📋 ID copiado'));
});
$('#settRequestNotif')?.addEventListener('click', () =>
  Notification.requestPermission().then(p => toast(p === 'granted' ? '🔔 Notificaciones activadas' : '❌ Permiso denegado', p === 'granted' ? '' : 'error'))
);
...
$('#settDeleteStatuses')?.addEventListener('click', async () => {
  try { await setDoc(doc(db, 'userStatuses', CU.uid), {stories:[]}); myStatuses = []; renderStoriesBar(); toast('🗑️ Estados eliminados'); } catch { toast('❌ Error', 'error'); }
});
$('#settSignOut')?.addEventListener('click', async () => { await setOffline(); await signOut(auth); location.href = 'index.html'; });

/* ── Music ── */
window.toggleMusic = function() {
  const a = $('#bg-music'), b = $('#musicBtn'); if (!a || !b) return;
...
setInterval(() => {
  const pool = BOTS.filter(c => c.id !== currentId); if (!pool.length) return;
