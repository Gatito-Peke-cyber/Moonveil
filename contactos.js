/* =========================================================
   Moonveil Portal — Contactos (JS OPTIMIZADO)
   - Funcionalidad corregida
   - Mejor manejo de scroll
   - Optimización de rendimiento
   - Estados y música mejorados
   ========================================================= */
...
/* =========================================================
   DATASET de Contactos (mantenido del original)
   ========================================================= */
const contacts = [
  {
    id: 'c1',
  ...
  },
  {
    ...
    unread: 1,
    kind: 'censored',
    quickReplies: ['Flechas', 'Hola', 'Como estas'],
    profile: {
      correo: 'brun.tallow@moonveil.mv',
      seccion: 'B-1',
      profesion: 'Flechero',
      pasatiempos: ['Tiro con Arco'],
      mascotas: ['Loro "Gatt"'],
      colorFav: 'Azul Cielo'
    },
    brain: {
  ...
  },
  {
    ...
    profile: {
      ...
      seccion: 'B-2',
      profesion: 'Cartografo',
      pasatiempos: ['Mapeo', 'Exploracion'],
      mascotas: ['Zorro "Brist"'],
      colorFav: 'Amarillo Vainilla'
    },
    brain: {
      keywords: {
  ...
  },
  {
    ...
    profile: {
      ...
      pasatiempos: ['Escribir', 'Leer'],
      mascotas: ['Vaca "Rodolf"'],
      colorFav: 'Rosa'
    },
    brain: {
      keywords: {
  ...
  },
  {
    ...
    profile: {
      correo: 'guau@moonveil.mv',
      seccion: 'G-UAU',
      profesion: 'Ser perron 😎',
  ...
  },
  {
    ...
    quickReplies: ['Hola'],
    profile: {
      correo: 'jack.gl@moonveil.mv',
      seccion: '?',
      profesion: 'The Teacher',
      pasatiempos: ['Enseñar'],
...
function renderContacts(list = contacts) {
  ...
  contactList.innerHTML = items || `<li class="muted" style="padding:10px;text-align:center">Sin resultados</li>`;
...
function contactItem(c) {
  ...
  const unread = c.unread ? `<span class="unread" aria-label="${c.unread} mensajes no leídos">${c.unread > 99 ? '99+' : c.unread}</span>` : '';
...
$$('.emoji-header button').forEach(btn => {
...
$('#btnAttach')?.addEventListener('click', () => toast('Adjuntar - No implementado 😸'));
...
btnMute?.addEventListener('click', () => {
  if (!currentId) return toast('Selecciona un contacto');
  if (muted.has(currentId)) {
    muted.delete(currentId);
...
function toast(msg) {
  ...
  toastEl._id = setTimeout(() => toastEl.classList.remove('show'), 1800);
}
...
const estados = [
  ...
  },
  {
    ...
    text: '🟨 Última hora desde el Overworld:<br>Durante la madrugada, varios creakings fueron vistos<br>merodeando zonas residenciales.',
    start: '2025-12-01',
    end: '2026-01-01',
    seen: false
  }
...
function renderStoriesBar() {
  ...
  contacts.forEach(c => {
    ...
    div.innerHTML = `<img src="${c.avatar}" alt="${c.name}">`;
...
function renderStory(s) {
  if (s.type === 'image')
    return `${s.url ? `<img src="${s.url}">` : ''}<p>${s.text || ''}</p>`;
...
$('.next-story')?.addEventListener('click', nextStory);
$('.prev-story')?.addEventListener('click', prevStory);
